...
//    _titleLabelBackgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
    ...
    _pageControlDotSize = CGSizeMake(16, 16);
...
-(void)setPageControlImage:(UIImage *)pageControlImage
{
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        TAPageControl *pageControl = (TAPageControl *)_pageControl;
        pageControl.dotImage = pageControlImage;
    }
}
-(void)setCurrentPageControlImage:(UIImage *)currentPageControlImage
{
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        TAPageControl *pageControl = (TAPageControl *)_pageControl;
         pageControl.currentDotImage = currentPageControlImage;
    }
    
    
}
-(void)setSpacingBetweenDots:(int)spacingBetweenDots
{
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        TAPageControl *pageControl = (TAPageControl *)_pageControl;
...
- (void)setupPageControl
{
    ...
    switch (self.pageControlStyle) {
        case SDCycleScrollViewPageContolStyleAnimated:
        {
            ...
            if (self.currentPageControlImage) {
                pageControl.currentDotImage = self.currentPageControlImage;

            }
            
            if (self.pageControlImage) {
                pageControl.dotImage = self.pageControlImage;
...
- (void)layoutSubviews
{
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        ...
        size = CGSizeMake(self.imagesGroup.count * self.pageControlDotSize.width * 0.5, self.pageControlDotSize.height);
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_biaoqianGroup.count) {
        cell.biaoqiantitle = _biaoqianGroup[itemIndex];
        if ([cell.biaoqiantitle isEqualToString:@""]) {
            cell.biaoqianLabel.alpha = 0;
        }else
        {
            cell.biaoqianLabel.alpha = 1;
    ...
    if (_titlesGroup.count) {
        ...
        if ([_titlesGroup[itemIndex] isEqualToString:@""]) {
            cell.backGroudView.alpha = 0;
        }else
        {
            cell.backGroudView.alpha = 1;
